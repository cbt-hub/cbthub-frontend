<script lang="ts">
  ...
  /** 이전 문제로 이동 */
  function prev(): void {
    location.href = `/round/${roundId}?questionId=${question.questionMeta.prev}`;
  }

  /** 다음 문제로 이동 */
  function next(): void {
    location.href = `/round/${roundId}?questionId=${question.questionMeta.next}`;
  }
   ...
   * TODO: 이전 문제, 다음 문제의 버튼이 고정되어 있어야 합니다. 
   * - 아래에 있을 지, 그냥 맨 위로 둘지 고민이다.
   * 
   * ✔️ 가장 첫번째 문제와 마지막 문제에서는 이전 문제, 다음 문제 버튼이 비활성화 되는 로직이 필요합니다.
   * ✔️ 다음문제, 이전문제 버튼을 누르면, 다음 문제로 넘어가거나 이전 문제로 넘어갑니다.
   * ✔️ 정답 확인 버튼을 누르면, 정답 확인 글씨가 정답 접기 글씨로 바뀌고 해설이 보이지 않게 바꿉니다.
   *
   * 추후 만들고 싶은 기능!
   ...
   *
   * 최적화!
   * TODO: 문제를 불러올 때, 미리 전 후 다섯 개의 문제를 불러와 사용자가 빠르게 문제를 탐색할 수 있게 합니다.
   */
  ...
  async function checkAnswer(): Promise<void> {
    ...
    try {
      ...
      correctDetails.set(
        question.details
          .map((detail: Detail, index: number) => ({ detail, index }))
          .filter(({ detail }: { detail: Detail }) => detail.isCorrect)
      );
    } catch (error) {
...
<div class="flex justify-center flex-col w-3/5 mt-16">
  ...
  <div class="button-container flex justify-between">
    ...
    <div class="flex">
      {#if question.questionMeta.prev === null}
        <button
          on:click={prev}
          class="text-white text-lg rounded-full bg-gray-400 px-3 py-2 mr-3 shadow-md"
      ...
      {#if question.questionMeta.next === null}
        <button
          on:click={next}
          class="text-white text-lg rounded-full px-3 py-2 shadow-md flex items-center bg-gray-400"
          disabled
      ...
      {:else}
        <button
          on:click={next}
          class="text-white text-lg rounded-full px-3 py-2 shadow-md flex items-center bg-blue-400"
          ><p class="mr-2">다음 문제</p>
          <i class="fa-solid fa-arrow-right"></i>
        </button>
      {/if}
